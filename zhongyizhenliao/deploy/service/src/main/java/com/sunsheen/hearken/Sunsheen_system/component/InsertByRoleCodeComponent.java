...
import com.sunsheen.jfids.hearken.component.login.dto.RolePermissionAssignmentDTO;
...
@Service("InsertByRoleCodeComponent")
@BixComponentPackage(dirname = "system/ai", type = "SYSTEM")
public class InsertByRoleCodeComponent extends ABaseComponent {

    private final SystemManagerStrategyFactory strategyFactory;

    public InsertByRoleCodeComponent(SystemManagerStrategyFactory strategyFactory) {
        this.strategyFactory = strategyFactory;
    }

    @Override
    @WebRemote(paramsType = {"data", "json"})
    @Component(name = "InsertByRoleCodeComponent", memo = "保存角色权限", type = "data")
    ...
    public Object run(Map param) {
        ...
        RolePermissionAssignmentDTO assignment = new RolePermissionAssignmentDTO();
        assignment.setRoleCode(SystemManagementParams.string(data.get(0), "role_code"));
        assignment.setResourceIds(data.stream()
                .map(item -> SystemManagementParams.string(item, "resource_id"))
                .filter(java.util.Objects::nonNull)
                .toList());
        strategyFactory.getResourceManager().replaceRolePermissions(assignment);
        return true;
    }
}
