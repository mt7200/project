...
<script setup lang="ts">
  ...
  const handleAddAuth = (row: AppRouteRecord): void => {
    //@SS  确保 parentId 是字符串类型
    parentId.value = row.id?.toString() || '0'
  ...
  const handleSubmit = async (formData: any): Promise<void> => {
    try {
      if (formData.menuType === 'menu') {
        const menuData = {
          parentPath: parentId.value || editData.value?.meta?.parentPath || '0', //SS 父菜单路径
          name: formData.label,
          path: formData.path,
        ...
        if (formData.id && formData.id > 0) {
          await fetchUpdateMenu(formData.id, buttonData)
          ElMessage.success('更新权限成功')
        } else {
          await fetchCreateMenu(buttonData)
