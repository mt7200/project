  ...
  import { ElMessageBox, ElMessage, ElForm, ElFormItem, ElInput, ElButton, ElTable, ElTableColumn, ElDialog, ElTree, ElCard, ElTreeSelect } from 'element-plus'
  ...
  <template>
    <div class="organization-manage-page art-full-height">
      <ElCard class="card-header" shadow="hover" style="margin-bottom: 20px;">
        <div class="flex justify-between">
          <ElForm :inline="true" :model="orgForm" ref="formRef" label-width="auto" class="form-layout">
            <div class="form-left">
              <ElFormItem class="form-item">
                <ElInput v-model="orgForm.text" placeholder="组织名称" clearable />
              </ElFormItem>
              <ElFormItem class="form-item">
                <ElTreeSelect v-model="orgForm.belongto" :data="allOrgTree" :props="treeProps" node-key="id"
        ...
        <div class="flex-1 h-0 mt-5">
          <ElTable :data="orgTree" row-key="id" :tree-props="{ children: 'children' }"
            class="organization-table">
            <ElTableColumn prop="text" label="组织名称" min-width="180" show-overflow-tooltip />

            <ElTableColumn label="电话" width="150" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.data?.tel }}
              </template>
            </ElTableColumn>

            <ElTableColumn label="邮箱" width="200" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.data?.email }}
              </template>
            </ElTableColumn>

            <ElTableColumn label="排序" width="80" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.data?.sequence }}
              </template>
            </ElTableColumn>

            <ElTableColumn label="备注" min-width="240" show-overflow-tooltip>
              <template #default="{ row }">
                {{ row.data?.memo }}
              </template>
            </ElTableColumn>

            <ElTableColumn label="状态" width="110" show-overflow-tooltip>
              <template #default="{ row }">
                <span :class="[
                  'status-label',
                  row.data?.enable == 1
                  ...
                  {{ row.data?.enable == 1 ? '启用' : '禁用' }}
                </span>
              </template>
            </ElTableColumn>

            <ElTableColumn label="操作" width="380" align="center" header-align="center">
              <template #default="{ row }">
                <span :style="{
                  color: row.data?.enable == 1 ? '#f56c6c' : '#67c23a',
                  cursor: 'pointer',
                  marginRight: '12px',
                  fontSize: '14px'
                }" @click="handleToggleStatus(row?.data || {})">
                  {{ row.data?.enable == 1 ? '禁用' : '启用' }}
                </span>

                <span style="color:#409eff;cursor:pointer;margin-right:12px;font-size:14px;"
                  @click="handleEdit(row?.data || {})">
                  修改
                </span>
                ...
                <span style="color:#e6a23c;cursor:pointer;font-size:14px;" @click="handleAdjustLayer(row?.data || {})">
                  层级调整
                </span>
              </template>
            </ElTableColumn>
        ...
        <ElDialog title="调整所属父节点" v-model="adjustDialogVisible" width="520px" class="tree-dialog" destroy-on-close>
          <div class="tree-dialog-content">
            ...
            <div class="tree-box">
              <ElTree :data="allOrgTree" :props="treeProps" node-key="id" :current-node="selectedParent"
                default-expand-all highlight-current @node-click="handleSelectParent" />
            </div>
          </div>
          <template #footer>
            <ElButton @click="handleClearParentSelection">选择顶层</ElButton>
            <ElButton @click="handleCancelAdjust">取消</ElButton>
            <ElButton type="primary" @click="handleConfirmAdjust">确定</ElButton>
          </template>
        </ElDialog>
      </ElCard>
