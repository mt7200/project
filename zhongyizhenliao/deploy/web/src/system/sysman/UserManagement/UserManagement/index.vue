...
<template>
  <div class="organization-manage-page">
    <ElCard class="card-header" shadow="hover">
      <!-- 查询/表单区域（统一模板风格） -->
      <ElForm :inline="true" :model="userForm" ref="formRef" class="form-layout">
        <div class="form-left">
          <ElFormItem class="form-item">
            <ElInput v-model="userForm.user_name" placeholder="姓名" clearable />
          </ElFormItem>

          <ElFormItem class="form-item">
            <ElInput v-model="userForm.account" placeholder="账号" clearable />
          </ElFormItem>
      ...
      <div class="flex-1 h-0 mt-5">
        <ElTable :data="userList" height="100%" class="organization-table">
          <ElTableColumn prop="user_name" label="姓名" width="120" />
          <ElTableColumn prop="account" label="账号" width="120" />
          <ElTableColumn prop="department" label="所属单位" min-width="140" />
          ...
          <ElTableColumn label="操作" width="260" align="center">
            <template #default="{ row }">
              <span
                style="color: #409eff; cursor: pointer; margin-right: 12px; font-size: 14px"
                @click="handleEdit(row)"
              >
                修改
              </span>
              <span
                style="color: #f56c6c; cursor: pointer; margin-right: 12px; font-size: 14px"
