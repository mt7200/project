<template>
  <ss:HViewPort class="view-port-box">
    <template #north>
      <!-- 搜索栏 -->
      <div class="w-full flex items-center justify-between" :attrs="{ height: 56 }">
        <div class="flex-1 flex items-center">
          ...
          />
          <div>
            <ss:HButton type="primary" @click="handleSizeChange">
              <ss:HIcon>
                <Search />
              </ss:HIcon>
              查询
        ...
        <div class="add-button-container">
          <ss:HButton type="primary" @click="handleCreate">
            <ss:HIcon>
              <Plus />
            </ss:HIcon>
            新增
    ...
    <template #center>
      <!-- 表格 -->
      <ss:HGridPanel :data="tableData" height="100%" v-loading="tableLoading" border>
        <ss:HGridColumn prop="role_id" label="角色id" show-overflow-tooltip />
        <ss:HGridColumn prop="role_name" label="名称" show-overflow-tooltip />
        <ss:HGridColumn prop="sort" label="排序" show-overflow-tooltip />
        <ss:HGridColumn prop="remark" label="备注" show-overflow-tooltip />
        <ss:HGridColumn label="操作" show-overflow-tooltip width="200">
        ...
        <template #paging>
          <ss:HGridPaging
            :dataUrl="`${VITE_API_URL}/data/SystemRolePage.json?scope=maintenance&role_id=${searchForm.roleId}&role_name=${searchForm.name}&sort=${searchForm.sort}`"
...
import type { Role, SearchForm, PageInfo, HGridPagingInstance } from "./type";
  ...
  ElMessageBox.confirm(`确定要删除角色"${row.role_name}"吗？`, "确认删除", {
    ...
    if (typeof validRes === "string") {
      ...
      ElMessageBox.confirm(
        `当前角色"${row.role_name}"下存在人员，是否继续删除角色？`,
        "确认删除",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
...
const handleEnable = async (row: Role) => {
  ...
  ElMessageBox.confirm(`确定要${action}角色"${row.role_name}"吗？`, `确认${action}`, {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
