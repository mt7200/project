...
import { ElMessageBox, ElMessage, ElForm, ElFormItem, ElInput, ElButton, ElTable, ElTableColumn, ElCard } from 'element-plus'
...
<template>
  <div class="user-access-page">
    <ss:HCard class="card-header" shadow="hover">
      ...
      <!-- 查询区域（对齐组织管理页） -->
      <ss:HForm :inline="true" :model="searchForm" ref="formRef" class="form-layout">

        <ss:HFormItem class="form-item">
          <ss:HInput v-model="searchForm.user_name" placeholder="姓名" clearable />
        </ss:HFormItem>

        <ss:HFormItem class="form-item">
          <ss:HInput v-model="searchForm.account" placeholder="账号" clearable />
        </ss:HFormItem>
      ...
      <div class="table-wrapper">
        <ss:HTable :data="userList" row-key="id" stripe class="user-table">

          <ss:HTableColumn prop="account" label="账号" width="120" />
          <ss:HTableColumn prop="user_name" label="姓名" width="120" />
          <ss:HTableColumn prop="department" label="所在单位" min-width="160" />

          <ss:HTableColumn label="是否入网" width="110">
          ...
          <ss:HTableColumn label="操作" width="220" align="center" header-align="center">
            <template #default="{ row }">
              <span :style="{
              ...
              <span style="color:#409eff;cursor:pointer;font-size:14px;" @click="handleResetPassword(row)">
                重置密码
              </span>
            </template>
          </ss:HTableColumn>
