          ...
          {currentStep === 1 ? (
            <div className="ws-card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <div className="ws-card-body" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
                  {prescriptionMock.formulas.map((f, i) => (
                    <div
                      ...
                      style={{
                        ...
                        border: !isCustomFormula && selectedFormulaIdx === i ? '1.5px solid var(--primary)' : '1px solid #E0E0E0',
                        background: !isCustomFormula && selectedFormulaIdx === i ? 'var(--primary-bg)' : '#FAFAFA',
                      }}
                    >
                      <div className={'ws-ai-formula-name' + (!isCustomFormula && selectedFormulaIdx === i ? ' active' : '')}>
                        {f.name}
                      </div>
                  ...
                  <div
                    onClick={() => {
                      ...
                      setAiHerbs([])
                      setFormulaEffects([])
                      setEditFormulaName(customFormulaName)
                      setEditReason('')
                      setDeletedHerb(null)
                      setEditingFormulaName(false)
                      setEditingEffectIdx(null)
                    ...
                    style={{
                      ...
                      border: isCustomFormula ? '1.5px solid var(--primary)' : '1px dashed #B5B5B5',
                      background: isCustomFormula ? '#F0FFF4' : '#FAFAFA',
                      display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3,
                    }}
                  >
                    <span style={{ fontSize: 14, lineHeight: 1, color: isCustomFormula ? 'var(--primary)' : 'var(--text-muted)' }}>+</span>
                    <span style={{ fontSize: 11, fontWeight: isCustomFormula ? 600 : 400, color: isCustomFormula ? 'var(--primary)' : 'var(--text-muted)' }}>自定义</span>
                    {isCustomFormula && <span style={{ fontSize: 9, background: '#FFF7E6', color: '#D46B08', padding: '0 6px', borderRadius: 4 }}>自定义</span>}
                  </div>
                </div>
                ...
                {/* 通用：方剂名双击编辑（仅AI方剂） */}
                {!isCustomFormula && editingFormulaName && (
                  <div style={{ display: 'flex', gap: 4, justifyContent: 'center', marginBottom: 6 }}>
                    <button style={{ border: '1px solid var(--primary)', background: 'var(--primary)', color: '#fff', borderRadius: 4, fontSize: 10, padding: '1px 10px', cursor: 'pointer' }} onClick={() => setEditingFormulaName(false)}>✓ 确认</button>
                    <button style={{ border: '1px solid #999', background: '#fff', color: '#999', borderRadius: 4, fontSize: 10, padding: '1px 10px', cursor: 'pointer' }} onClick={() => { setEditingFormulaName(false); setEditFormulaName(prescriptionMock.formulas[selectedFormulaIdx].name) }}>× 取消</button>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 8 }}>
                  {aiHerbs.length === 0 && (
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', padding: '4px 0' }}>暂未添加药材，点击下方添加</div>
                  )}
                  {aiHerbs.map((h, i) => (
                    editingHerbIdx === i ? (
                      <div key={i} style={{ display: 'flex', gap: 4, alignItems: 'center', padding: '2px 6px', background: '#fff', border: '1px solid var(--primary)', borderRadius: 6 }}>
                        <input style={{ width: 70, border: 'none', outline: 'none', fontSize: 12, padding: '2px 0' }} value={editName} onChange={e => setEditName(e.target.value)} placeholder="药名" />
                        <input style={{ width: 50, border: 'none', outline: 'none', fontSize: 12, padding: '2px 0', textAlign: 'right' }} value={editDosage} onChange={e => setEditDosage(e.target.value)} placeholder="剂量" />
                        <button style={{ border: 'none', background: 'var(--primary)', color: '#fff', borderRadius: 4, fontSize: 10, padding: '1px 6px', cursor: 'pointer' }} onClick={() => { if (editName.trim()) { const updated = [...aiHerbs]; updated[i] = { name: editName.trim(), dosage: editDosage.trim() }; setAiHerbs(updated) }; setEditingHerbIdx(null) }}>✓</button>
                        <button style={{ border: 'none', background: 'transparent', color: '#999', cursor: 'pointer', fontSize: 12 }} onClick={() => setEditingHerbIdx(null)}>×</button>
                        <button style={{ border: 'none', background: '#FF4D4F', color: '#fff', borderRadius: 4, fontSize: 10, padding: '1px 6px', cursor: 'pointer' }} onClick={() => { setDeletedHerb({ herb: aiHerbs[i], index: i }); setAiHerbs(prev => prev.filter((_, idx) => idx !== i)); setEditingHerbIdx(null); if (undoTimeoutRef.current) clearTimeout(undoTimeoutRef.current); undoTimeoutRef.current = setTimeout(() => setDeletedHerb(null), 6000) }}>删</button>
                      </div>
                    ) : (
                      <span key={i} className="ws-tag" style={{ fontSize: 12, background: '#F0FFF4', borderColor: '#95DE64', padding: '3px 10px', cursor: 'pointer' }} onClick={() => { setEditingHerbIdx(i); setEditName(h.name); setEditDosage(h.dosage) }}>
                        {h.name} <strong>{h.dosage}</strong>
                      </span>
                    )
                  ))}
                  <span className="ws-tag" style={{ fontSize: 12, background: '#FAFAFA', borderColor: '#D9D9D9', padding: '3px 10px', cursor: 'pointer', color: 'var(--text-muted)' }} onClick={() => { setAiHerbs(prev => [...prev, { name: '', dosage: '' }]); setEditingHerbIdx(aiHerbs.length); setEditName(''); setEditDosage('') }}>
                    + 添加药材
                  </span>
                </div>
                ...
                <div className="ws-sub-title" style={{ fontSize: 11 }}>功效 <span style={{ fontWeight: 400, color: 'var(--text-muted)', fontSize: 10 }}>点击编辑，双击添加</span></div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
                  {formulaEffects.map((e, i) => (
                    editingEffectIdx === i ? (
                      <div key={i} style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                        <input
                          style={{ width: 70, border: '1px solid var(--primary)', borderRadius: 4, fontSize: 11, padding: '1px 4px', outline: 'none' }}
                          value={editEffect}
                          onChange={e2 => setEditEffect(e2.target.value)}
                          autoFocus
                        />
                        <button style={{ border: 'none', background: 'var(--primary)', color: '#fff', borderRadius: 4, fontSize: 9, padding: '1px 5px', cursor: 'pointer' }} onClick={() => { if (editEffect.trim()) { const updated = [...formulaEffects]; updated[i] = editEffect.trim(); setFormulaEffects(updated) }; setEditingEffectIdx(null) }}>✓</button>
                        <button style={{ border: 'none', background: '#FF4D4F', color: '#fff', borderRadius: 4, fontSize: 9, padding: '1px 5px', cursor: 'pointer' }} onClick={() => { setFormulaEffects(prev => prev.filter((_, idx) => idx !== i)); setEditingEffectIdx(null) }}>×</button>
                      </div>
                    ) : (
                      <span key={i} style={{ padding: '2px 8px', background: '#FFF7E6', borderRadius: 4, border: '1px solid #FFD591', cursor: 'pointer', fontSize: 11 }} onClick={() => { setEditingEffectIdx(i); setEditEffect(e) }}>
                        {e}
                      </span>
                    )
                  ))}
                  <span style={{ padding: '2px 8px', background: '#FAFAFA', borderRadius: 4, border: '1px dashed #D9D9D9', cursor: 'pointer', fontSize: 11, color: 'var(--text-muted)' }} onClick={() => { setFormulaEffects(prev => [...prev, '']); setEditingEffectIdx(formulaEffects.length); setEditEffect('') }}>
                    + 添加功效
                  </span>
                </div>
                ...
                {editingReason ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 8 }}>
                    <textarea
                      style={{ fontSize: 11, lineHeight: 1.6, padding: '6px 10px', background: '#fff', border: '1px solid var(--primary)', borderRadius: 6, resize: 'vertical', outline: 'none', width: '100%', boxSizing: 'border-box' }}
                      value={editReason}
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      ...
                      <button style={{ border: '1px solid #999', background: '#fff', color: '#999', borderRadius: 4, fontSize: 10, padding: '2px 12px', cursor: 'pointer' }} onClick={() => { setEditingReason(false); setEditReason(isCustomFormula ? '' : prescriptionMock.formulas[selectedFormulaIdx].reason) }}>× 取消</button>
                    </div>
                  </div>
                ) : (
                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.6, padding: '6px 10px', background: 'var(--bg-page)', borderRadius: 6, cursor: 'pointer' }} onClick={() => { setEditingReason(true); setEditReason(isCustomFormula ? '' : prescriptionMock.formulas[selectedFormulaIdx].reason) }}>
                    {isCustomFormula ? (editReason || '点击此处输入推荐理由...') : prescriptionMock.formulas[selectedFormulaIdx].reason}
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
              ...
              <div className="ws-card" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="ws-card-body" style={{ flex: 1, fontSize: 12, padding: '10px 14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 2 }}>
